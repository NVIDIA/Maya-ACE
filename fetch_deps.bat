...
if not exist %BASE_DIR%venv (
    ...
    %PYTHON% -m venv %BASE_DIR%venv --upgrade-deps
    if errorlevel 1 (
...
call python -m pip install -r %BASE_DIR%deps\python-deps.pip.txt -q --disable-pip-version-check --target %BASE_DIR%_build\python-deps
if errorlevel 1 (
...
call %PACKMAN% pull -v -t config=%BUILD_CONFIG% --platform windows-x86_64 %BASE_DIR%deps\build-deps.packman.xml
if errorlevel 1 (
    echo Failed to pull dependencies in build-deps.packman.xml
    exit /b 1
)
call %PACKMAN% pull -v -t config=%BUILD_CONFIG% --platform windows-x86_64 %BASE_DIR%deps\target-deps.packman.xml
if errorlevel 1 (
